...
Date: 22/04/2023
This is the airline_satisfaction_trainer.py module.
Usage:
...
from airline_satisfaction_transform import (
    CATEGORICAL_FEATURES,
    LABEL_KEY,
    NUMERICAL_FEATURES,
    transformed_name,
)
from airline_satisfaction_tuner import input_fn
...
def get_model(hyperparameters, show_summary=True):
    ...
    Args:
        hyperparameters (kt.HyperParameters): hyperparameters
    ...
    deep = layers.Dense(
        hyperparameters['dense_units'], activation=tf.nn.relu)(concatenate)

    for _ in range(hyperparameters['num_layers']):
        deep = layers.Dense(
            hyperparameters['dense_units'], activation=tf.nn.relu)(deep)
